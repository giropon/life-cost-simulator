...
import { calcAnnualEventsMonthly, formatYen } from '../../utils/calculations'
import type { AnnualEvent } from '../../types'

const MONTHS = ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月']

function EventForm({ initial, onClose }: { initial?: AnnualEvent; onClose: () => void }) {
  const { addAnnualEvent, updateAnnualEvent } = useCostStore()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const data = { name, costPerOccurrence: Number(cost), month: Number(month) }
    if (initial) updateAnnualEvent(initial.id, data)
    else addAnnualEvent(data)
    onClose()
  }

  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-indigo-200 p-4 space-y-3">
      <h3 className="text-sm font-semibold text-slate-700">{initial ? '年次イベントを編集' : '年次イベントを追加'}</h3>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="text-xs text-slate-500 mb-1 block">開催月</label>
...
export default function AnnualEventsSection() {
  const { annualEvents, removeAnnualEvent } = useCostStore()
  const [editing, setEditing] = useState<AnnualEvent | null>(null)
  const [showForm, setShowForm] = useState(false)
  const monthly = calcAnnualEventsMonthly(annualEvents)

  const sorted = [...annualEvents].sort((a, b) => a.month - b.month)

  return (
    <div className="p-4 max-w-2xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-800">年次イベント</h2>
          <p className="text-sm text-slate-500">毎年特定の月に行う予定のイベントを登録します</p>
        </div>
      ...
      {annualEvents.length === 0 ? (
        <div className="text-center py-10 text-slate-400">
          ...
          <p className="text-sm">年次イベントがまだ登録されていません</p>
        </div>
      ) : (
        <>
                ...
                <li key={e.id} className="flex items-center justify-between px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      毎年{e.month}月 · {formatYen(e.costPerOccurrence)}
                      <span className="ml-1 text-slate-400">（月割: {formatYen(e.costPerOccurrence / 12)}）</span>
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => removeAnnualEvent(e.id)}
          ...
          <div className="bg-indigo-50 rounded-xl p-4 flex justify-between items-center">
            <span className="text-sm font-medium text-indigo-700">年次イベント合計（月額換算）</span>
